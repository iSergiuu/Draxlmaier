...
export default function MyComplaints() {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center text-brand-primary font-bold bg-brand-bg">Se încarcă tichetele...</div>;

    return (
        <div className="min-h-screen bg-brand-bg p-8 transition-colors duration-300 relative">
            <div className="max-w-5xl mx-auto space-y-6">
                
                <button onClick={() => navigate('/dashboard')} className="flex items-center text-brand-primary hover:opacity-80 font-medium transition-colors mb-4">
                ...
                <div className="bg-brand-card p-6 rounded-xl shadow-sm border border-brand-border flex justify-between items-center transition-colors duration-300">
                    <div>
                        <h1 className="text-2xl font-bold text-brand-text flex items-center">
                            <Ticket className="w-7 h-7 mr-3 text-brand-primary" /> Problemele Mele
                        </h1>
                ...
                {!error && complaints.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {complaints.map((ticket) => {
                            ...
                            return (
                                <div key={ticket.id} className="bg-brand-card p-6 rounded-xl shadow-sm border border-brand-border flex flex-col justify-between hover:border-brand-primary hover:shadow-md transition-all duration-300">
                                    <div>
                                        <div className="flex justify-between items-start mb-4">
                                            <span className="text-xs font-mono font-bold text-brand-muted bg-brand-bg border border-brand-border px-2 py-1 rounded">
                                                #{ticket.ticketNumber || ticket.id.substring(0,6).toUpperCase()}
                                            </span>
                                            {getPriorityBadge(ticket.priority)}
                                        </div>
                                        <h2 className="text-lg font-bold text-brand-text mb-2 line-clamp-1">{ticket.title}</h2>
                                        <p className="text-sm text-brand-muted line-clamp-2 mb-4">{ticket.description}</p>
                                    </div>
                                    ...
                                    <div className="border-t border-brand-border pt-4 mt-2">
                                        ...
                                        <button 
                                            ...
                                            className="w-full py-2 bg-brand-bg hover:opacity-80 border border-brand-border text-brand-text font-medium rounded-lg transition-colors flex items-center justify-center"
                                        >
                                            Detalii Tichet
                                        </button>
                                    </div>
