...
import { Ticket, Loader2, Inbox, UserCheck, Circle, AlertCircle, CheckCircle2, Flame } from 'lucide-react';
...
// Tabul "Tichetele mele" = statusuri pe care adminul le-a atins
...
export default function AdminTickets() {
    ...
    useEffect(() => {
        fetchAll();
        // Polling la 30s ca sa apara tickete noi automat
    ...
    const processed = tickets.filter(t => {
        ...
        if (activeTab === 'MINE') {
            ...
            // Tabul ALL: toate ticketele (inclusiv NEW)
        }
            ...
            <div className="flex gap-1 border-b border-brand-border">
                {[
                    ...
                    <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-medium border-b-2 transition-colors ${activeTab === tab.key ? 'border-brand-primary text-brand-primary' : 'border-transparent text-brand-muted hover:text-brand-text'}`}>
